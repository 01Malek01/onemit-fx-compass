...
const MarginControls: React.FC<MarginControlsProps> = ({
  ...
  return <Card className="fx-card relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Slider id="usd-margin-slider" min={0} max={10} step={0.1} value={[parseFloat(usdMarginInput) || 0]} onValueChange={handleUsdSliderChange} disabled={isLoading} />
            </div>
            <div className="flex gap-2">
              <Input id="usdMargin" type="number" value={usdMarginInput} onChange={handleUsdMarginChange} disabled={isLoading} min="0" max="100" step="0.1" className="text-right" />
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Slider id="other-margin-slider" min={0} max={10} step={0.1} value={[parseFloat(otherMarginInput) || 0]} onValueChange={handleOtherSliderChange} disabled={isLoading} />
            </div>
            <div className="flex gap-2">
              <Input id="otherMargin" type="number" value={otherMarginInput} onChange={handleOtherMarginChange} disabled={isLoading} min="0" max="100" step="0.1" className="text-right" />
              <span className="flex items-center text-muted-foreground text-sm min-w-[16px]">%</span>
            </div>
          </div>
        </div>
        
        <Button onClick={handleUpdate} className="w-full mt-6 relative overflow-hidden group" disabled={isLoading || isSaved}>
          {isSaved ? <>
